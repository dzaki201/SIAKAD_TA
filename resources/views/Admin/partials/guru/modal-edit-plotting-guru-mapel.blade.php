<div id="edit-plotting-guru-mapel-modal-{{ $guru->id }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-2xl md:h-auto mx-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Edit Plotting Guru Mata Pelaajaran</h3>
                <button type="button"
                    class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                    data-modal-hide="edit-plotting-guru-mapel-modal-{{ $guru->id }}">
            ...
            <div class="p-6 space-y-6">
                <form action="{{ route('admin.edit.plotting.guru-mapel', ['id' => $guru->id]) }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        ...
                        <select id="mata_pelajaran_id" name="mata_pelajaran_id" required
                            ...
                            @foreach ($mapels as $mapel)
                                <option value="{{ $mapel->id }}" @if ($guru->mata_pelajaran_id && $guru->mata_pelajaran_id == $mapel->id) selected @endif>
                                    {{ $mapel->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="text-right border-t border-gray-200 dark:border-gray-600 pt-4 space-x-4">
                        ...
                        <button data-modal-hide="edit-plotting-guru-mapel-modal-{{ $guru->id }}" type="button"
